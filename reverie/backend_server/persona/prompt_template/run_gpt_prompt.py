...
def run_gpt_prompt_task_decomp(persona, task, duration):
    """
    Run GPT prompt for task decomposition with better error handling
    """
    default_task = [[task, duration]]  # Default fallback
    
    try:
        prompt = generate_prompt([task, duration], 
                               f"{curr_dir}/prompt_template/task_decomp.txt")
        
        def __func_validate(gpt_response, prompt=None): 
            try:
                decomp_list = [i.strip() for i in gpt_response.split("\n") if i.strip()]
                return bool(decomp_list)  # Return True if we have valid items
            except:
                return False
            ...
            try:
                ...
                for i in decomp_list:
                    try:
                        ...
                        if len(k) < 2:
                            continue
                        duration_str = ''.join(c for c in k[1] if c.isdigit())
                        if not duration_str:
                            continue
                        ...
                        if not task_desc:
                            continue
                        ret.append([task_desc, task_duration])
                    except:
                        continue
                
                # If no valid decomposition, return default
                if not ret:
                    return default_task
                    
                # Adjust durations to match total
                ftime_sum = sum(item[1] for item in ret)
                if ftime_sum != duration and ret:
                    diff = duration - ftime_sum
                    # Distribute the difference across all tasks if possible
                    if len(ret) > 0:
                        per_task_adjustment = diff // len(ret)
                        remainder = diff % len(ret)
                        for i in range(len(ret)):
                            ret[i][1] += per_task_adjustment
        ...
        # Ensure we always return a valid list with at least one task
        ...
        print(f"Task decomposition error: {str(e)}")
        return default_task
